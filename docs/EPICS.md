...
**E6 — Leave**
- ✅ **RESOLVED 2026-06-15 — HR-assigned entitlement.** Move from the auto-apply-to-all rule engine to **HR-assigned per-employee entitlement**: HR enrols each employee into specific leave types with a quota (manual per-type at hire); only assigned types are requestable; the **eligibility gates are dropped** (`INV-7` gender/notice/min-service/lifetime). The **per-type `cap_basis` window + reset** mechanics are **kept** (cap_basis reinterpreted as reset cadence); `entitled` sources from a new `employee_leave_entitlements` table instead of `cap_value`/agreement. **Supersedes** the gate rules in the per-type decision below (LQ-15 / INV-7) and the auto-apply behaviour. Spec: [leave-entitlement-assignment PRD](epics/E6-leave/prds/leave-entitlement-assignment.md). **Shipped 2026-06-15 — backend, API (`GET/POST/PATCH/DELETE /employees/{id}/leave-entitlements`; balance endpoint now INNER JOINs entitlements so it returns only assigned types), and frontend all live; the gates are retired (gate columns kept as unenforced metadata).**
