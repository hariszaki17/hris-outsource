...
Overtime in Indonesia is tiered by day type — a workday hour, a rest-day hour, and a public-holiday hour carry different multipliers. hris-outsource needs configurable **OT rules** that classify and (for future payroll) weight OT by day type, **global only** (one rule per day type), with a minimum-duration threshold and a pre-approval flag. In v1 the multipliers are **reference only** (we record hours, not pay).

## 2. Goals & non-goals

**Goals**
- Define OT rules with day-type tiers (workday / rest day / holiday) — **global only** — `min_minutes`, and `requires_preapproval`.
...
| Ref | Rule |
|-----|------|
| OR-1 | A rule defines: `day_type` (Workday/RestDay/Holiday), `multiplier` (reference), `min_minutes`, `requires_preapproval`. Rules are **global** — exactly one per `day_type`. |
...
`OvertimeRule`: `id, name, day_type, multiplier, min_minutes, requires_preapproval, status`.
...
Feature: Overtime rules
  ...
  Scenario: One global rule per day type
    Given a global Workday rule exists
    When OT is classified for any placement on a workday
    Then the single global Workday rule applies
...
| C-3 | No rule for a day type | Fallback to a default + flag. |
| C-4 | Recurring vs one-off holidays | `recurring` handles annual fixed dates; movable holidays added yearly. |

## 9. Dependencies

E4 (schedule → rest-day), E1 (audit), F7.2/F7.4 (consumers), holiday calendar master.

## 10. Decisions & open questions

- ✅ Day-type tiers, **global only** (one rule per day type), multipliers = reference (hours-only v1). *(Service-line scope + precedence dropped 2026-06-12 — service line removed project-wide.)*
