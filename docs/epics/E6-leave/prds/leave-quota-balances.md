...
| LQ-8 | **Pro-rate** the `ANNUAL_POOL` grant for mid-year joiners — `entitled ≈ entitlement × remaining_months / 12` (half-up), based on the employee's `join_at` date. Probation-period employees also receive pro-rated annual leave. Other windows are not pro-rated (statutory caps apply in full). *(Resolved 2026-06-19 — ANNUAL_POOL prorated based on join date.)* |
| LQ-9 | **Cancel leave** — a leave request may be cancelled **only when no one has approved it yet** (i.e., zero approval actions on the E11 instance). Once any line member has approved, cancellation is blocked. *(Resolved 2026-06-19.)* |
