...
> **Epic:** E3 Placement Management · **Feature:** F3.4 · **Status:** Draft v2
...
Each **client company** runs its on-site team under **exactly one shift leader** — the person who builds rosters, verifies attendance, and approves leave/overtime there. The shift leader is themselves an agent placed within that company, granted elevated authority scoped to it. Leader scope is **company-wide** — there is no per-site leader. This PRD owns **designating, reassigning, and vacating** that role, and is the gate that turns an ordinary agent into the company's approver. *(Resolved 2026-06-19 — leader scope company-wide only.)*

## 2. Goals & non-goals

**Goals**
- Assign exactly one shift leader per **company** (company-wide, INV-2).
- Enforce that the leader is an agent **actively placed within that company** (INV-4) and leads **only that one company** (INV-3).
...
| Ref | Rule |
|-----|------|
| SL-1 | A company has **at most one active** shift-leader assignment (INV-2) — uniqueness is on `client_company_id`. |
| SL-2 | The candidate must have an **active placement within that company** (INV-4). |
...
| SL-6 | When the leader's **placement at the company ends** (end/terminate/resign/transfer — F3.2/F3.3), their assignment is **auto-vacated** and a vacancy is raised. |
...
|-------|------|-------|
| `id` | PK | |
| `client_company_id` | FK | the company; **unique among active** (SL-1) |
| `employee_id` | FK | the leader; must have active placement within the company (SL-2) |
...
> `site_id` removed *(2026-06-19 — leader scope is company-wide only)*. RBAC scope: the `shift_leader` role + company scope are **derived at request time** from the active assignment by server middleware (SL-10), **not persisted on `users`**; they take effect on assign and lapse on `unassigned_at`, both effective on the next request (E1).
...
| C-2 | Outgoing leader is also being transferred (F3.3) | Vacate is idempotent — transfer-vacate and reassign-end converge to one ended record. |
| C-3 | Two HR admins assign different leaders to the same company concurrently | Unique-active constraint (SL-1) makes the second commit fail; retried as a reassignment. |
| C-4 | Company archived while it has a leader | Leader assignment vacated; company no longer accepts placements (F3.1 BR-3). |
| C-5 | Self-assignment (HR admin who is also placed there) | Allowed if invariants hold; audited. |
...
**Resolved (2026-06-19 — leader scope company-wide):**
- ✅ **Leader scope is company-wide only** — `leader_scope` removed from `ClientCompany`; `ShiftLeaderAssignment.site_id` removed. Shift leader always covers the entire company. No per-site leaders.
...
- ✅ Leader must be actively placed **within the company**.
