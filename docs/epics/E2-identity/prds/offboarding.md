...
| **OB-3** | **`closed_reason` ∈ {`END_OF_TERM`, `RESIGNED`, `TERMINATED`, `RETIRED`, `DECEASED`, `ABSCONDED`, `OTHER`}.** `TERMINATED` and `ABSCONDED` (mangkir) require a free-text reason; `OTHER` requires a note. *(Extends the prior 4-value set `END_OF_TERM`/`RESIGNED`/`TERMINATED`/`OTHER` — see §10.)* **Reconciled (2026-06-07):** the **MVP reason set is the 4 CHECK-supported values** `END_OF_TERM`/`RESIGNED`/`TERMINATED`/`OTHER` (the agreement `closed_reason` CHECK constraint only allows these four; reason is **required** at offboard). The extended `RETIRED`/`DECEASED`/`ABSCONDED` values are **DEFERRED** pending a `closed_reason` CHECK-constraint migration (§10). |
...
> **Implementation note — OB-1 cascade & traceability (2026-06-07).** The offboard action (`berhentikan`, `POST /employees/{id}:deactivate`) is now **implemented atomically** in a single transaction: it sets `Employee.status = inactive`, **closes** the active `EmploymentAgreement` (`status = closed` + `closed_reason` + `closed_at`), **ends every non-terminal `Placement`** (terminal `lifecycle_status` + `ended_reason` + `ended_at`), and **disables the linked `User` + revokes its sessions** (OB-9). Cascaded records are **closed (terminal), never deleted**.
> **Traceability:** each cascaded close carries an audit marker `caused_by: "employee_offboard"` + `source_employee_id`, and the parent `employee.offboard` audit entry **lists the cascaded agreement id + placement ids**. This makes it unambiguous whether an agreement/placement closed as a **cascade** from offboarding vs a **direct standalone close** (which carries no such marker). Records are CLOSED (terminal state), never deleted (OB-12).
> **Reason→placement-terminal mapping:** `RESIGNED → RESIGNED`, `TERMINATED → TERMINATED`, `END_OF_TERM`/`OTHER → ENDED`.
...
**Resolved (2026-06-07):**
- ✅ **Offboard cascade is implemented atomically with audit traceability** (OB-1, §5 implementation note) — one transaction sets the employee inactive, **closes** the active agreement (`closed`/`closed_reason`/`closed_at`), **ends all non-terminal placements** (terminal `lifecycle_status`/`ended_reason`/`ended_at`), and disables the user + revokes sessions (OB-9). Each cascaded close is tagged `caused_by = employee_offboard` + `source_employee_id`, and the parent `employee.offboard` audit entry lists the cascaded agreement + placement ids — distinguishing a **cascade** from a **direct standalone close** (no marker). Records are CLOSED (terminal), never deleted.
- ✅ **MVP reason set = the 4 CHECK-supported values** `END_OF_TERM`/`RESIGNED`/`TERMINATED`/`OTHER` (required at offboard). The extended `RETIRED`/`DECEASED`/`ABSCONDED` (OB-3) are **DEFERRED** — the `closed_reason` CHECK constraint only allows the four; enabling them needs a CHECK-constraint migration.
- ✅ **Reason→placement-terminal mapping:** `RESIGNED → RESIGNED`, `TERMINATED → TERMINATED`, `END_OF_TERM`/`OTHER → ENDED`.
