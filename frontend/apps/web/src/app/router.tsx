...
import { ChangePasswordScreen } from '@/features/auth/change-password-screen.tsx';
...
import {
  PeriodCloseScreen,
  type PeriodCloseSearch,
} from '@/features/e8-payroll/period-close-screen.tsx';
...
// Change password — authenticated, but lives OUTSIDE the app shell (like login/reset) so
// the EP-3 forced temp-password rotation is a clean, distraction-free screen. The token is
// already set at this point; the mutation is authorized. Unauthenticated hits bounce to login.
...
const authedRoute = createRoute({
  ...
  beforeLoad: ({ location }) => {
    ...
    // Funnel every authed route to /change-password until the flag clears.
...
// RECON §2: sibling route under authedRoute; typed search mirrors attendanceDashboardRoute pattern.
// ROUTE_REQUIREMENTS /^\/payroll/ → 'payroll.read' already covers this path.
const periodCloseRoute = createRoute({
  getParentRoute: () => authedRoute,
  path: '/payroll/period-close',
  component: PeriodCloseScreen,
  validateSearch: (search: Record<string, unknown>): PeriodCloseSearch => {
    const out: PeriodCloseSearch = {};
    if (typeof search.year === 'number' && search.year >= 2000 && search.year <= 2100)
      out.year = search.year;
    else if (typeof search.year === 'string' && /^\d{4}$/.test(search.year))
      out.year = Number(search.year);
    if (typeof search.month === 'number' && search.month >= 1 && search.month <= 12)
      out.month = search.month;
    else if (typeof search.month === 'string' && /^\d{1,2}$/.test(search.month)) {
      const m = Number(search.month);
      if (m >= 1 && m <= 12) out.month = m;
    }
    if (
      typeof search.tab === 'string' &&
      ['kehadiran', 'lembur', 'cuti', 'ringkasan'].includes(search.tab)
    )
      out.tab = search.tab as PeriodCloseSearch['tab'];
    if (typeof search.cursor === 'string' && search.cursor) out.cursor = search.cursor;
    return out;
  },
});
